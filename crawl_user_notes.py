...
class Data_Spider():
    ...
    def spider_user_all_note(self, user_url: str, cookies_str: str, base_path: dict, save_choice: str, require_num: int = 200, excel_name: str = '', proxies=None):
        """
        爬取一个用户的所有笔记
        ...
        :param require_num: 需要爬取的笔记数量，默认200条
        ...
        note_list = []
        try:
            success, msg, all_note_info = self.xhs_apis.get_user_all_notes(user_url, require_num, cookies_str, proxies)
            if success:
            ...
            success = False
            msg = e
        logger.info(f'爬取用户笔记 {user_url}: {success}, msg: {msg}')
        return note_list, success, msg

def spider_user_notes(user_url: str, cookies_str: str, require_num: int = 200, save_choice: str = 'all'):
    """
    爬取用户的所有笔记
    :param user_url: 用户主页链接
    :param cookies_str: cookies字符串
...
if __name__ == "__main__":
    ...
    for user_id in user_ids[1:]:
        ...
        print(f"\n开始爬取用户 {user_id} 的笔记...")
        print(f"计划爬取数量：{require_num}条")
        
        # 调用爬虫函数
        note_list, success, msg = spider_user_notes(
            user_url=user_url,
            cookies_str=cookies_str,
            require_num=require_num
        )
        ...
        print(f"信息：{msg}")
        if note_list:
            print(f"实际获取到 {len(note_list)} 条笔记")
        else:
            print("未获取到任何笔记")
